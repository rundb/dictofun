...
#define BLE_FTS_MAX_RX_CHAR_LEN        BLE_FTS_MAX_DATA_LEN        /**< Maximum length of the RX Characteristic (in bytes). */
#define BLE_FTS_MAX_TX_CHAR_LEN        BLE_FTS_MAX_DATA_LEN        /**< Maximum length of the TX Characteristic (in bytes). */

// generated on https://www.uuidgenerator.net/
// eb3caea4-0db1-11ec-82a8-0242ac130003
#define FTS_BASE_UUID                  {{0xEB, 0x3C, 0xAE, 0xA4, 0x0D, 0xB1, 0x11, 0xEC, 0x82, 0xA8, 0x02, 0x42, 0xAC, 0x13, 0x00, 0x03}} 
...
#define FTS_ENABLE_PIN_DEBUGGING 0

#if(FTS_ENABLE_PIN_DEBUGGING == 1)
#define FTS_DEBUG_PIN_SET(_pin) nrf_gpio_pin_set(DBG_PIN_ ## _pin)
#define FTS_DEBUG_PIN_CLR(_pin) nrf_gpio_pin_clear(DBG_PIN_ ## _pin)
#else
#define FTS_DEBUG_PIN_SET(_pin) 
#define FTS_DEBUG_PIN_CLR(_pin) 
#endif

static void fts_enable_gpio_debug(void)
{
#if(FTS_ENABLE_PIN_DEBUGGING == 1)
...
static void on_write(ble_fts_t * p_fts, ble_evt_t const * p_ble_evt)
{
    ...
    if (
        (p_evt_write->handle == p_fts->tx_handles.cccd_handle)
        &&
        (p_evt_write->len == 2)
       )
    {
        ...
        (p_evt_write->handle == p_fts->file_info_handles.cccd_handle)
        &&
        (p_evt_write->len == 2)
       )
    {
        ...
        else
        {
            p_fts->is_info_char_notification_enabled = false;
             ...
             (p_evt_write->handle == p_fts->rx_handles.value_handle)
             &&
...
{
    uint8_t data_buf[1 + sizeof(ble_fts_ble_params_info_t)];
    ble_gatts_hvx_params_t hvx_params;

    VERIFY_PARAM_NOT_NULL(p_fts);

    if ((p_fts->conn_handle == BLE_CONN_HANDLE_INVALID) || (!p_fts->is_info_char_notification_enabled))
    {
        return NRF_ERROR_INVALID_STATE;
    }

    uint16_t length = 1 + sizeof(ble_fts_ble_params_info_t);

    data_buf[0] = 2;
    memcpy(&data_buf[1], ble_params_info, sizeof(ble_fts_ble_params_info_t));
...
uint32_t ble_fts_send_file(ble_fts_t * p_fts, uint8_t * p_data, uint32_t data_length, uint32_t max_packet_length)
{
    ...
    file_info.file_size_bytes = data_length;
    ble_fts_file_info_send(p_fts, &file_info);
