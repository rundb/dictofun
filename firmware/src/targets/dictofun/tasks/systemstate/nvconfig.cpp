...
static void fstorage_evt_handler(nrf_fstorage_evt_t* p_evt)
{
    ...
    case NRF_FSTORAGE_EVT_WRITE_RESULT: {
        NRF_LOG_DEBUG(
            "--> Event received: wrote %d bytes at address 0x%x.", p_evt->len, p_evt->addr);
    }
    break;

    case NRF_FSTORAGE_EVT_ERASE_RESULT: {
        NRF_LOG_DEBUG(
            "--> Event received: erased %d page from address 0x%x.", p_evt->len, p_evt->addr);
    }
    break;
...
result::Result NvConfig::load(Configuration& config)
{
    ...
    const auto read_result =
        nrf_fstorage_read(&fstorage, fstorage_start_address, &tmp, sizeof(Configuration));
...
result::Result NvConfig::store(Configuration& config)
{
    ...
    const auto erase_result =
        nrf_fstorage_erase(&fstorage, fstorage_start_address, fstorage_size, NULL);
    ...
    config.crc = crc32_compute(
        reinterpret_cast<const uint8_t*>(&config), sizeof(Configuration) - sizeof(uint32_t), NULL);

    // program the configuration block
    const auto write_result =
        nrf_fstorage_write(&fstorage, fstorage_start_address, &config, sizeof(Configuration), NULL);
...
result::Result NvConfig::apply_default_config()
{
    ...
    default_config.crc = crc32_compute(reinterpret_cast<const uint8_t*>(&default_config),
