...
constexpr uint32_t APP_ADV_DURATION{
...
result::Result BleSystem::configure()
{
    ...
    // todo: wrap this into "init_dfu()"
    dfus_init.evt_handler = ble_dfu_evt_handler;

    const auto dfu_init_result = ble_dfu_buttonless_init(&dfus_init);
    if (NRF_SUCCESS != dfu_init_result)
    {
        NRF_LOG_ERROR("DFU: initialization has failed (err code: %d)", dfu_init_result);
...
void BleSystem::register_fs_communication_queues(QueueHandle_t commands_queue,
