...
void NrfI2c::irq_handler(nrf_drv_twi_evt_t const* p_event)
{
    ...
    case NRF_DRV_TWI_EVT_DONE: {
        ...
        {
            const auto read_result = read(_transaction_context.address,
                                          _transaction_context.rx_data,
                                          _transaction_context.rx_data_size);
...
i2c::Result NrfI2c::init()
{
    nrf_drv_twi_config_t twi_config{
        _config.clk_pin, _config.data_pin, get_frequency_config(_config.baudrate), 1, false, false};
    const auto init_result =
        nrf_drv_twi_init(&_twi_instance, &twi_config, twi_event_handler, &_irq_context);
...
{
    const auto rx_result = nrf_drv_twi_rx(&_twi_instance, address, data, size);
...
{
    const auto tx_result = nrf_drv_twi_tx(&_twi_instance, address, data, size, false);
...
i2c::Result NrfI2c::write_read(const uint8_t address,
                               const uint8_t* const tx_data,
                               const uint8_t tx_size,
                               uint8_t* rx_data,
                               const uint8_t rx_size)
{
    const auto tx_result = nrf_drv_twi_tx(&_twi_instance, address, tx_data, tx_size, true);
    if(NRFX_SUCCESS != tx_result)
    {
        return Result::ERROR_GENERAL;
    }
