...
public:
    ...
    {
        static_assert(sizeof(RawSampleType) == 4 * sizeof(CodedSampleType),
                      "Wrong types configuration for ADPCM");
    }
    ...
    result::Result decode(CodedSampleType& input, RawSampleType& output) override
    {
...
private:
    ...
    static constexpr uint32_t max_codec_descriptor_size{
        std::min(static_cast<uint32_t>(sizeof(CodedSampleType)), static_cast<uint32_t>(16UL))};
};
