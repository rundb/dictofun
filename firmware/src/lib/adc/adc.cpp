...
result::Result NrfAdc::init()
{
    ...
    for(auto i = 0; i < _config.count; ++i)
    {
        ...
        nrf_saadc_channel_config_t _adc_channel_config =
            NRFX_SAADC_DEFAULT_CHANNEL_CONFIG_SE(analog_input_id);
        const auto channel_init_result =
            nrfx_saadc_channel_init(analog_input_id, &_adc_channel_config);
...
result::Result NrfAdc::get_measurement(const AnalogInputId pin_id, Measurement& measurement)
{
    ...
    const auto sample_result =
        nrfx_saadc_sample_convert(_config.analog_input_ids[0], &measurement_i16);
    if(NRFX_SUCCESS != sample_result)
    {
        return result::Result::ERROR_GENERAL;
    }
    measurement = static_cast<float>(measurement_i16) * ADC_BASE_VOLTAGE /
                  static_cast<float>(ADC_12_BIT_MAX_VALUE);
...
void adc_event_handler(nrfx_saadc_evt_t const* p_event)
{
    ...
    case NRFX_SAADC_EVT_DONE: {
        NrfAdc::instance().conversion_completed(p_event->data.done.size,
